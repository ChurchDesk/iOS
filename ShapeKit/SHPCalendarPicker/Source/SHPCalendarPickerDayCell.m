...
@property (nonatomic, strong) CAShapeLayer *statusCircleShapeLayer;
@end
...
- (void)addSubLayers {
    ...
    [self.layer addSublayer:self.statusCircleShapeLayer];
}
...
- (void)setFrame:(CGRect)frame {
    ...
    [self updateStatusCircleShapeLayerPath];
}
...
- (void)updateStatusCircleShapeLayerPath {
    CGFloat fraction = 0.18;
    CGFloat radius = fraction*CGRectGetHeight(self.frame)/2.0f;
    CGFloat offsetX = CGRectGetWidth(self.frame)/2-radius;
    CGFloat offsetY = CGRectGetHeight(self.frame)-radius-2;
    CGRect roundedRect = CGRectMake(offsetX, offsetY, 2.0f * radius, 2.0f * radius);
    UIBezierPath *path = [UIBezierPath bezierPathWithRoundedRect:roundedRect cornerRadius:radius];

    [self.statusCircleShapeLayer setPath:path.CGPath];
}
...
- (CAShapeLayer *)statusCircleShapeLayer {
    if (!_statusCircleShapeLayer) {
        _statusCircleShapeLayer = [CAShapeLayer new];
        _statusCircleShapeLayer.fillColor = [UIColor redColor].CGColor;
    }
    return _statusCircleShapeLayer;
}
...
- (void)setStatusColor:(UIColor *)statusColor {
    ...
    [CATransaction begin];
    [CATransaction setValue:(id)kCFBooleanTrue forKey:kCATransactionDisableActions];
    self.statusCircleShapeLayer.fillColor = statusColor.CGColor;
